...
int main(void) {
    ...
    const int SCREEN_WIDTH = 10, SCREEN_HEIGHT = n;
    ...
    for (;;) {
        for (int i = 0; i < PIECE_COUNT; i++)
            for (int y = 0; y < SCREEN_HEIGHT; y++)
                for (int x = 0; x < SCREEN_WIDTH; x++) {
                    int c = grid_dfs(g, (Point) { x, y }, '1' + i);
                    ...
                    for (int py = 0; py < SCREEN_HEIGHT; py++)
                        for (int px = 0; px < SCREEN_WIDTH; px++)
                            if (g->ptr[py][px] == '1' + i && g->visited[py][px])
                                g->ptr[py][px] = '0', g->visited[py][px] = 0;
                }
        ...
        for (;;) {
            ...
            for (int y = 0; y < SCREEN_HEIGHT - 1; y++)
                for (int x = 0; x < SCREEN_WIDTH; x++)
                    if (g->ptr[y][x] != '0' && g->ptr[y + 1][x] == '0') {
                        g->ptr[y + 1][x] = g->ptr[y][x], g->ptr[y][x] = '0';
