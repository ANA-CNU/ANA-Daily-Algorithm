...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        for(int i=0;i<K;i++) {
            ...
            String passFrom=x.nextToken();
            String passTo=x.nextToken();

            root.mv(passFrom,passTo);
        }

        int M=Integer.parseInt(br.readLine());

        for(int i=0;i<M;i++){
           Directory targetDirectory = root.cd(root.getLeafName(br.readLine()));
...
class Root {
    ...
    void mv(String from, String to){
        String parentNameOfFrom=getParentName(from);
        String childNameOfFrom=getLeafName(from);

        Directory parentDirectoryOfFrom=cd(parentNameOfFrom);
        Directory childDirectoryOfFrom=cd(childNameOfFrom);

        Directory targetDirectory=cd(getLeafName(to));

        parentDirectoryOfFrom.childrenDirectories.remove(childNameOfFrom);
        parentDirectoryOfFrom.directoryNames.remove(childNameOfFrom);

        targetDirectory.childrenDirectories.putAll(childDirectoryOfFrom.childrenDirectories);
        targetDirectory.filesInDirectory.putAll(childDirectoryOfFrom.filesInDirectory);

        targetDirectory.directoryNames.addAll(childDirectoryOfFrom.directoryNames);
    }
    ...
    String getLeafName(String pass){
        String[] args=pass.split("/");
        return args[args.length-1];
    }

    String getParentName(String pass){
        String[] args=pass.split("/");
